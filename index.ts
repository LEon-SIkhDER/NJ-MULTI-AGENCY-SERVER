...
async function run() {
    try {
        ...
        app.patch("/pitchers/assign-moderator", async (req, res) => {
            const { pitcherUids, moderatorUid } = req.body as { pitcherUids: string[], moderatorUid: string }
            // set moderatorUid for newly assigned pitchers
            const result = await pitchersCollection.updateMany(
                { uid: { $in: pitcherUids } },
                { $set: { moderatorUid } }
            )
            res.send(result)
        ...
        app.patch("/pitchers/unassign-moderator", async (req, res) => {
            const { pitcherUid } = req.body as { pitcherUid: string }
            const result = await pitchersCollection.updateOne(
                { uid: pitcherUid },
                { $set: { moderatorUid: "" } }
            )
            res.send(result)
        ...
        app.patch("/task/:id", async (req, res) => {
            ...
            const query = { _id: new ObjectId(id) }
            delete rest._id
            let update: any = { $set: { ...rest, updatedAt: new Date() } }
            if (status) {
        ...
        app.get("/admin/overview-stats", async (req, res) => {
            ...
            allTasks.forEach(task => {
                ...
                if (s === "completed" || s === "complete") completedTasks++
                else if (s === "rejected" || s === "reject") rejectedTasks++
                else if (s === "postponed" || s === "postpone") postponedTasks++
                else pendingTasks++
            ...
            const moderatorRankings = allModerators.map(m => {
                const mTasks = allTasks.filter(t => 
                    t.assignedBy === m.uid || t.assignedByUid === m.uid || t.moderatorUid === m.uid || t.createdBy === m.uid
                )
        ...
        app.get("/moderator/overview-stats", async (req, res) => {
            ...
            const modTasks = allTasks.filter(t => 
                t.assignedBy === moderatorUid || t.assignedByUid === moderatorUid || t.moderatorUid === moderatorUid || t.createdBy === moderatorUid
            )
        ...
        app.get("/admin/payments", async (req, res) => {
            ...
            employees.sort((a, b) => {
                if (a.unpaidBalance > 0 && b.unpaidBalance <= 0) return -1
                if (b.unpaidBalance > 0 && a.unpaidBalance <= 0) return 1
                if (a.unpaidBalance !== b.unpaidBalance) return b.unpaidBalance - a.unpaidBalance
        ...
        app.patch("/admin/pay-employee", async (req, res) => {
            ...
            if (role === "pitcher") {
                const result = await tasksCollection.updateMany(
                    { assignedTo: employeeUid, paymentStatus: "paid", pitcherPaidStatus: { $ne: "paid" } },
                    { $set: { pitcherPaidStatus: "paid", pitcherPaidAt: new Date() } }
                )
